...
function normalizeCoordinateSource(source) {
  ...
  const precision = Math.max(2, Math.min(5, Number(source.precision) || 5));
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
  return { latitude, longitude, precision };
}

function matchesCoordinateSource(lat, lon, source) {
  const normalized = normalizeCoordinateSource(source);
  if (!normalized) return false;
  return Number(lat).toFixed(normalized.precision) === normalized.latitude.toFixed(normalized.precision)
    && Number(lon).toFixed(normalized.precision) === normalized.longitude.toFixed(normalized.precision);
}

function matchesGeometryEntity(lat, lon, entity) {
  if (!entity || typeof entity !== 'object') return false;
  const sources = Array.isArray(entity.sources) ? entity.sources : [];
  if (sources.length) {
    return sources.some((source) => matchesCoordinateSource(lat, lon, source));
  }
  return matchesRule(lat, lon, entity);
}

function findMatchingGeometryEntity(lat, lon, entities) {
  for (const entity of entities || []) {
    if (matchesGeometryEntity(lat, lon, entity)) return entity;
  }
  return null;
}
...
module.exports = {
  ...
  matchesCoordinateSource,
  matchesGeometryEntity,
  findMatchingGeometryEntity,
  buildRuleAddress,
};
