...
const { compareProviderAddresses } = require('./address-validation');
...
async function reverseGeocode(lat, lon, config, options = {}) {
  ...
  if (mappedState) address.state = mappedState;
  if (mappedCity) address.city = mappedCity;

  if (preferBuildingName && address.poiName && address.city && !address.city.includes(address.poiName)) {
  ...
  const result = {
    ...
    summary: {
      ...
      providerAgreement: compareProviderAddresses(vworldAddress, naverAddress),
    },
    providers: {
      vworld: vworldAddress ? {
        country: vworldAddress.country || '',
