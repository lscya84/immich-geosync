...
async function listRules() {
  ...
  const rules = ruleRes.rows.map((row) => {
    ...
    return { ...rule, assetCount };
  });

  return rules.sort((a, b) => (b.assetCount - a.assetCount) || (a.priority - b.priority) || String(a.name).localeCompare(String(b.name), 'ko'));
}
...
async function listClusters(bounds = {}) {
  ...
  adminDebug('listClusters', {
    ...
    singleRuleClusters: clusters.length,
