...
const { buildSpatialGridKey } = require('./spatial-clustering');
const { getWorkerSnapshot } = require('./worker-monitor');

const pool = new Pool(getDbConfig());
const adminDebugLogs = /^(1|true|yes|on)$/i.test(String(process.env.ADMIN_DEBUG_LOGS || '').trim());
const LOCKED_COORDINATE_PROPERTIES = ['latitude', 'longitude'];
...
function hasLockedCoordinate(row) {
  const locked = Array.isArray(row?.lockedProperties) ? row.lockedProperties : [];
  return locked.some((property) => LOCKED_COORDINATE_PROPERTIES.includes(String(property || '').trim()));
}
...
const clusterAssetSelectSql = `
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE af."assetId" = asset."id"
      AND af."type" = 'preview'
    ORDER BY
      CASE WHEN af."isEdited" = asset."isEdited" THEN 0 ELSE 1 END,
      af."updatedAt" DESC
    LIMIT 1
  ) preview ON TRUE
  LEFT JOIN LATERAL (
...
async function ensureClusterGeocodeCacheTable() {
  ...
  for (const [column, type] of structuredColumns) {
    await pool.query(`ALTER TABLE "custom_geo_geocode_cache" ADD COLUMN IF NOT EXISTS "${column}" ${type}`);
  }
}
...
async function upsertClusterGeocodeCache(cacheKey, state, city) {
  ...
  await pool.query(`
    INSERT INTO "custom_geo_geocode_cache"
      ("cache_key", "country", "state", "city", "provider", "validation_status", "status", "failure_reason", "updated_at")
    VALUES ($1, '대한민국', $2, $3, 'manual', 'manual', 'success', '', CURRENT_TIMESTAMP)
...
async function listClusterGeocodeCaches({ page = 1, pageSize = 50, state = '', city = '' } = {}) {
  await ensureClusterGeocodeCacheTable();
  const normalizedPage = Math.max(1, Number(page) || 1);
  const normalizedPageSize = Math.max(10, Math.min(200, Number(pageSize) || 50));
  ...
  if (normalizedState) {
    ...
    where.push(`COALESCE("state", '') = $${params.length}`);
  }
  if (normalizedCity) {
    params.push(normalizedCity);
    where.push(`COALESCE("city", '') = $${params.length}`);
  }
  ...
  const listRes = await pool.query(`
    SELECT
      ...
      COALESCE("legal_dong", '') AS legal_dong,
      COALESCE("road_address", '') AS road_address,
      COALESCE("jibun_address", '') AS jibun_address,
    ...
    FROM "custom_geo_geocode_cache"
    ${whereSql}
    ORDER BY "updated_at" DESC NULLS LAST, "cache_key" ASC
    LIMIT $${params.length - 1} OFFSET $${params.length}
  `, params);

  const caches = listRes.rows.map((row) => ({
...
async function listClusterGeocodeCacheStates() {
  await ensureClusterGeocodeCacheTable();
  const res = await pool.query(`
    SELECT DISTINCT COALESCE("state", '') AS state
    FROM "custom_geo_geocode_cache"
    WHERE COALESCE("state", '') <> ''
    ORDER BY state ASC
  `);
  return res.rows.map((row) => row.state).filter(Boolean);
}

async function listClusterGeocodeCacheCities(state = '') {
  await ensureClusterGeocodeCacheTable();
  const normalizedState = String(state || '').trim();
  ...
  const res = await pool.query(`
    ...
    FROM "custom_geo_geocode_cache"
    WHERE COALESCE("state", '') = $1
      AND COALESCE("city", '') <> ''
    ORDER BY city ASC
  `, [normalizedState]);
  return res.rows.map((row) => row.city).filter(Boolean);
}

async function updateClusterGeocodeCache({ cacheKey, state = '', city = '', status = 'success', failureReason = '' } = {}) {
  const normalizedKey = String(cacheKey || '').trim();
  if (!normalizedKey) throw new Error('cacheKey는 필수입니다.');
  await ensureClusterGeocodeCacheTable();
  ...
  const res = await pool.query(`
    UPDATE "custom_geo_geocode_cache"
    SET
      "state" = $2,
      "city" = $3,
      "status" = $4,
      "failure_reason" = $5,
      "provider" = CASE WHEN $4 = 'success' THEN 'manual' ELSE "provider" END,
      "provider_agreement" = CASE WHEN $4 = 'success' THEN NULL ELSE "provider_agreement" END,
      "validation_status" = CASE WHEN $4 = 'success' THEN 'manual' ELSE $4 END,
      "validation_details" = CASE WHEN $4 = 'success' THEN '{"reason":"manual-review"}'::jsonb ELSE "validation_details" END,
      "updated_at" = CURRENT_TIMESTAMP
    WHERE "cache_key" = $1
    RETURNING "cache_key", COALESCE("state", '') AS state, COALESCE("city", '') AS city,
              COALESCE("status", '') AS status, COALESCE("failure_reason", '') AS failure_reason,
              COALESCE("provider", '') AS provider, COALESCE("validation_status", '') AS validation_status,
              "updated_at"
  `, [normalizedKey, normalizedState, normalizedCity, normalizedStatus, normalizedFailureReason]);

  if (!res.rows[0]) return null;
  return {
    cacheKey: res.rows[0].cache_key,
    state: res.rows[0].state,
    city: res.rows[0].city,
    status: res.rows[0].status,
    failureReason: res.rows[0].failure_reason,
    provider: res.rows[0].provider,
    validationStatus: res.rows[0].validation_status,
...
async function deleteClusterGeocodeCaches(cacheKeys = []) {
  ...
  await ensureClusterGeocodeCacheTable();
  const res = await pool.query(`DELETE FROM "custom_geo_geocode_cache" WHERE "cache_key" = ANY($1::varchar[])`, [normalized]);
...
function buildMergeSourceCacheKeys(source) {
  ...
  return [buildSpatialGridKey(normalized.latitude, normalized.longitude, getDefaultClusterRadiusMeters()).key];
}
...
async function getCoordinateClusterAssetIds(latitude, longitude, precision = 5) {
  ...
  const res = await pool.query(`
    SELECT "assetId", "lockedProperties"
    FROM "asset_exif" exif
    WHERE ROUND(CAST(exif."latitude" AS numeric), $3) = $1
      AND ROUND(CAST(exif."longitude" AS numeric), $3) = $2
  ...
  return {
    ...
    unlockedAssetIds: res.rows.filter((row) => !hasLockedCoordinate(row)).map((row) => row.assetId),
    lockedAssetIds: res.rows.filter((row) => hasLockedCoordinate(row)).map((row) => row.assetId),
    digits,
    latitude: lat,
    longitude: lon,
  };
...
async function updateClusterCoordinates({ latitude, longitude, precision = 5, nextLatitude, nextLongitude } = {}) {
  ...
  if (!cluster.unlockedAssetIds.length) {
    return {
      updatedCount: 0,
      skippedLockedCount: cluster.lockedAssetIds.length,
      latitude: normalizedNextLatitude,
      longitude: normalizedNextLongitude,
      precision: cluster.digits,
    };
  ...
  await pool.query(
    `UPDATE "asset_exif"
     SET "latitude" = $2,
         "longitude" = $3
     WHERE "assetId" = ANY($1::uuid[])`,
    [cluster.unlockedAssetIds, normalizedNextLatitude, normalizedNextLongitude],
  );
  ...
  adminDebug('updateClusterCoordinates', {
    ...
    precision: cluster.digits,
    updatedCount: cluster.unlockedAssetIds.length,
    skippedLockedCount: cluster.lockedAssetIds.length,
  });
...
async function getWorkerStatus(options = {}) {
  return getWorkerSnapshot(pool, options);
}
