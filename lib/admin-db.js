...
async function listClusters(bounds = {}) {
  ...
  const res = await pool.query(`
    SELECT
      ...
      MIN(exif."assetId")::text AS sample_asset_id,
    ...
    LIMIT $6
  `, [south, north, west, east, precision, limit]);
  ...
  return res.rows.map((row) => ({
    ...
    samplePreviewUrl: row.sample_asset_id ? `/api/assets/${row.sample_asset_id}/preview` : '',
    country: row.country,
    state: row.state,
    city: row.city,
  }));
...
async function getClusterAssets(latitude, longitude, limit = 12, precision = 5) {
  ...
  const res = await pool.query(`
    ...
    LIMIT $4
  `, [lat.toFixed(digits), lon.toFixed(digits), digits, Math.max(1, Math.min(30, Number(limit) || 12))]);
