...
async function getClusterAssets(latitude, longitude, limit = 12, precision = 5, offset = 0) {
  ...
  const res = await pool.query(`
    ...
    ORDER BY asset."fileCreatedAt" DESC
    LIMIT $4 OFFSET $5
  `, [lat.toFixed(digits), lon.toFixed(digits), digits, Math.max(1, Math.min(30, Number(limit) || 12)), pageOffset]);
...
async function getMergedClusterAssets(sources = [], limit = 12, offset = 0) {
  ...
  for (const source of Array.isArray(sources) ? sources : []) {
    ...
    const digits = Math.max(2, Math.min(5, Number(source?.precision) || 5));
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) continue;
    const latKey = lat.toFixed(digits);
    const lonKey = lon.toFixed(digits);
    for (const row of rows) {
      if (seen.has(row.asset_id)) continue;
      if (Number(row.latitude).toFixed(digits) === latKey && Number(row.longitude).toFixed(digits) === lonKey) {
        seen.add(row.asset_id);
        matched.push(row);
      }
    }
