...
async function queryClusterAssetRowsByRoundedGroups(groups = [], limit = 12, offset = 0) {
  ...
  const pageLimit = Math.max(1, Math.min(200, Number(limit) || 12));
  const pageOffset = Math.max(0, Number(offset) || 0);
  const params = [];
  const conditions = normalizedGroups.map((group) => {
    params.push(group.latitude.toFixed(group.precision));
    params.push(group.longitude.toFixed(group.precision));
    params.push(group.precision);
    const latIndex = params.length - 2;
    const lonIndex = params.length - 1;
    const precisionIndex = params.length;
    return `(ROUND(CAST(exif."latitude" AS numeric), $${precisionIndex}) = $${latIndex} AND ROUND(CAST(exif."longitude" AS numeric), $${precisionIndex}) = $${lonIndex})`;
  });
...
async function getMergedClusterAssets(sources = [], limit = 12, offset = 0) {
  ...
  for (const source of Array.isArray(sources) ? sources : []) {
    if (source?.ruleId) ruleSources.push(source);
    else coordinateSources.push(source);
  }

  const matched = coordinateSources.length
    ? [...await queryClusterAssetRowsByRoundedGroups(coordinateSources, Math.max(60, Number(limit) + Number(offset) + 24), 0)]
    : [];
  const seen = new Set(matched.map((row) => row.asset_id));

  if (ruleSources.length) {
    const rows = await queryAllClusterAssetRows();
    for (const source of ruleSources) {
      const rule = await getRule(source.ruleId);
      if (!rule) continue;
      for (const row of rows) {
        if (seen.has(row.asset_id)) continue;
        if (matchesRule(Number(row.latitude), Number(row.longitude), rule)) {
  ...
  matched.sort((a, b) => new Date(b.file_created_at).getTime() - new Date(a.file_created_at).getTime());
  return normalizeClusterAssetPage(matched, limit, offset);
}
