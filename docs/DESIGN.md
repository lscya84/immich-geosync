...
- 지도 도구 모음은 현재 표시용 적응형 격자를 선택적으로 확인할 수 있는 `표시 격자 보기` 토글을 제공해야 하며 기본값은 꺼짐이어야 한다.
- 사진 1장짜리 일반 위치 클러스터는 원형 마커가 아니라 작은 단색 점으로 표시하고, Polygon 규칙과 수동 클러스터의 관리용 마커 크기는 유지해야 한다.
...
`표시 격자 보기`를 켜면 현재 화면에서 일반 위치점 집계에 실제 사용된 격자만 얇은 반투명 경계로 표시한다. 이 오버레이는 지도 렌더링 성능을 위한 동적 표시 격자이며, worker의 15m 주소 처리 격자와는 별개다.
...
- 표시 격자 토글이 기본적으로 꺼져 있고, 켰을 때 현재 일반 위치점의 동적 격자만 표시되는지 확인
- 사진 1장짜리 일반 클러스터가 작은 점으로 표시되고 관리용 마커는 기존 크기를 유지하는지 확인
